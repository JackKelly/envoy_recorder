...
def _(pl):
    ...
    # Turn the wide format (where each device ID is its own column) to a tall and long format
    # (where there's a single device ID column, and a `stats` column which
    # holds a struct containing data for just that device ID.)
    ...
    # `channels` is a list of length 1 (because each IQ7+ micro-inverter only has a single PV panel connected to it.)
    # So, here, `explode("channels")` effectively just changes `channels` from a list containing one struct, into that struct (removing the list).
    df = df.explode("channels").unnest("channels")

    # The code that saves Envoy data to JSONL is deliberatly very simple. It doesn't check for duplicates.
    # It just grabs data from the Envoy and saves it to disk. So let's remove duplicates now.
    # 'sn' = 'serial number'
    # 'created' = the unix timestamp (in seconds) when the recording was created.
    ...
    # Convert the `flags_hex` string to a UInt64.
    # `flags` is almost certainly a 64-bit bit mask, because `flags_hex`
    # is a 16-character hex string. 16 chars x 4 bits per char = 64 bits.
    # We use the `flags_hex` instead of the `flags` int because it's possible that the
    # JSON parser will have used a float during processing. 
    # So it's safest to parse the hex string.
    ...
    # After all the jiggling around in the code above, the rows will be in arbitrary order,
    # so we have to sort them as a final step.
    # Sorting by serial number *first* should make the data easier to compress
    # because the values should change fairly smoothly, and Parquet is a columnar format.
    df = df.sort(["serial_number", "period_end_time"])

    # Force the column ordering (required for concat)
    df = df.select(
        [
            "serial_number",
            "period_end_time",
